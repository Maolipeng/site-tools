...
def build_parser() -> argparse.ArgumentParser:
    ...
    create_parser.add_argument("--listen-ipv6", action="store_true", help="Add explicit IPv6 listen directives to the Nginx config.")
    create_parser.add_argument("--upstream-host", help="Upstream host for proxy/node/systemd sites, for example 127.0.0.1, ::1, or localhost.")
    ...
    listen_ipv6_group = update_parser.add_mutually_exclusive_group()
    listen_ipv6_group.add_argument("--listen-ipv6", dest="listen_ipv6", action="store_true", help="Enable explicit IPv6 listen directives in the Nginx config.")
    listen_ipv6_group.add_argument("--no-listen-ipv6", dest="listen_ipv6", action="store_false", help="Disable explicit IPv6 listen directives in the Nginx config.")
    update_parser.set_defaults(listen_ipv6=None)
    update_parser.add_argument("--upstream-host", help="Override upstream host for proxy/node/systemd sites.")
    ...
    doctor_parser.add_argument("--domain", help="Optional domain used for IPv6 DNS and HTTPS readiness hints.")
    doctor_parser.add_argument("--type", choices=["node", "proxy", "static", "systemd"], help="Optional target site type for doctor suggestions.")
    doctor_parser.add_argument("--port", type=int, help="Optional target local port for doctor suggestions.")
    doctor_parser.add_argument("--upstream-host", help="Optional upstream host used in doctor suggestions.")
    doctor_parser.add_argument("--listen-ipv6", action="store_true", help="Assume the target site will enable explicit IPv6 listen directives.")
