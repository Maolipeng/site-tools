...
DEFAULT_PROJECT_ROOT="$(cd "$SKILL_DIR/../.." && pwd)"

usage() {
  cat <<'EOF'
Usage: install_skill.sh [options]
...
Options:
  ...
  --path PATH           Install to an explicit skills directory path
...
for target in "${expanded_targets[@]}"; do
  ...
  seen+=" $target "
  install_one "$target" "$scope" "$project_root" "$mode" "$explicit_path"
done
